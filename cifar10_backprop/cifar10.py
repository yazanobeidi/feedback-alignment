...
def inference(images, training=False):
  ...
  with tf.variable_scope('input') as scope:
    drop_input = tf.layers.dropout(inputs=images, rate=0.1, training=training)
    _activation_summary(drop_input)
  ...
  drop_pool1 = tf.layers.dropout(inputs=pool1, rate=0.25, training=training)
  ...
  drop_pool2 = tf.layers.dropout(inputs=pool2, rate=0.25, training=training)
  ...
  drop_pool3 = tf.layers.dropout(inputs=pool3, rate=0.5, training=training)
...
def train(total_loss, global_step):
  ...
  with tf.control_dependencies([loss_averages_op]):
    opt = tf.train.MomentumOptimizer(lr, 0.95)
    grads = opt.compute_gradients(total_loss)
    maxnorm = [(tf.clip_by_norm(grad, 4), var) for grad, var in grads]

  # Apply gradients.
  apply_gradient_op = opt.apply_gradients(maxnorm, global_step=global_step)
